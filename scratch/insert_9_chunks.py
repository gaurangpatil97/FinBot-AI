...
def insert_chunks():
    ...
    for c in chunks:
        ...
        if company not in clients:
            ...
            col_name = f"{company}_pdf_text"
            collection = client.get_collection(col_name)
            collections[company] = collection
            pre_counts[company] = collection.count()

    print("Pre-insertion counts:")
    for comp, count in pre_counts.items():
        print(f"  {comp}: {count}")

    for c in chunks:
        company = c["company"]
        source_path = os.path.join(base_path, company, "pdf", c["filename"])
        ...
        print(f"\nEmbedding chunk for {company} page {c['page']}...")
        embedded_chunks = generate_embeddings([chunk_dict])
        
        content_hash = hashlib.md5(c["content"].encode("utf-8")).hexdigest()
        doc_id = f"manual_table_{c['filename']}_{c['page']}_{content_hash[:8]}"
        
        collections[company].add(
            ids=[doc_id],
    ...
    for company, collection in collections.items():
        new_count = collection.count()
        diff = new_count - pre_counts[company]
        print(f"  {company}: {new_count} (Increased by {diff})")
        
        # Verify chunks exist
        res = collection.get(where={"chunk_type": "manual_table"})
        print(f"  Total manual_table chunks in {company}: {len(res['ids'])}")
